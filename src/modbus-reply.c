...
static const char *names[] = {
	...
	[MODBUS_FC_READ_FILE_RECORD] = "read_file_record",
	[MODBUS_FC_WRITE_FILE_RECORD] = "write_file_record",
...
int modbus_reply_callback(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    int verify_complete = 0; /* some functions do the verify inside the first switch-case,
								and set this to true to skip the generic one */
    ...
    case MODBUS_FC_WRITE_FILE_RECORD: {
		int off = offset + 1 ; // offset is at function

		uint8_t byte_count = req[off++];
		(void)byte_count;
		...
		for (;;) {
            ...
            if (off + 7 > req_length) {
                rsp_length = response_exception(
                    ...
                    "Encoded sub-request exceeds request-buffer size\n");
                break;
            }
            ...
            if (req[off++] != 6) {
                rsp_length = response_exception(
                    ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_VALUE, rsp, TRUE,
                    "Expected sub-request-type 6 for file_records - got %d\n",
                    req[off - 1]);
				break;
			}

			file_number = req[off++] << 8;
			file_number |= req[off++];

			if (file_number == 0) {
                rsp_length = response_exception(
                    ctx, &sft, MODBUS_EXCEPTION_ILLEGAL_DATA_ADDRESS, rsp, TRUE,
                    "Expected file_number shall be at least 1\n");
				break;
			}
            ...
            verified = ctx->reply_cb->verify(ctx->reply_user_ctx, slave, function, address | (file_number << 16), nb);
			if (verified != 0)
				break;

			if (!is_read) {
                ctx->reply_cb->write(ctx->reply_user_ctx, slave, function, address | (file_number << 16), nb * 2, &req[off]);
				off += nb * 2;
			}

            if (off == req_length) // done processing all sub-requests
				break;
		}
        max_nb = is_read ? MODBUS_MAX_READ_REGISTERS : MODBUS_MAX_WRITE_REGISTERS;
		verify_complete = 1;
    ...
    case MODBUS_FC_WRITE_MULTIPLE_REGISTERS:
        ...
        rc = ctx->reply_cb->write(ctx->reply_user_ctx, slave, function,
                                  address, nb, &req[offset + 6]);
